...
public class MiembrosService {
       ...
       if(miembrosRepository.existsByEmail(miebre.getEmail())){
        ...
        if(miebre.getFechaNacimiento() == null){
            ...
            throw new BusinessException("La fecha de nacimiento no puede ser futura");
                ...
                throw new BusinessException("El miembro debe ser mayor de edad");
        }
        EntrenadoresModel entrenador = entrenadorService.obtenerEntrenadorPorId(entrenadorId);
        if (entrenador.getEstado() != Estado.ACTIVO) {
            throw new BusinessException("El entrenador no está activo o no existe");
        }
        if (entrenador.getMiembros().size() >= 10) {
            throw new BusinessException("El entrenador ya tiene 10 miembros asignados");
    ...
    public MiembrosModel actualizarMiembro(Long id, MiembrosModel miembroinformacioAactualizar){
        ...
        if (miembroID.isEmpty()) {
            throw new ResourceNotFoundException("No existe este miembro");
        }
        ...
        public boolean actualizarEstadoMiembro(Long id, Estado nuevoEstado) {
            MiembrosModel miembroReal = miembrosRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Miembro no encontrado"));
